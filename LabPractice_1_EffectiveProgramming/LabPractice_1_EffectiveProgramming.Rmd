...
Name: Fernando Santana García ICAI ID: 2018004624 Collaborated with: Jorge Calvar
...
# YOUR CODE GOES HERE
t1 <- microbenchmark::microbenchmark(cs_for(x), cs_apply(x), cumsum(x))
t1
#Cumsum es el más rapido
```
...
# YOUR CODE GOES HERE
x2 <- 1:50000
t2 <- microbenchmark::microbenchmark(cs_for(x2), cs_apply(x2), cumsum(x2), time = 1)
t2
# The relative difference increases significantly between both values.
#
```
...
```{r q1c}
# YOUR CODE GOES HERE
profvis::profvis({x2 <- 1:50000
t2 <- microbenchmark::microbenchmark(cs_for(x2), cs_apply(x2), cumsum(x2), time = 1)
t2})
# Microbenchmark function is the bottleneck of this code. It is where I expected since the other lines are quick and easy to compute.
# YOUR CODE GOES HERE
```
...
# The function my.sum is the part that takes most time. The function sq.var is the one that requires most memory.
#
```
...
# YOUR CODE GOES HERE
#COMENTARRR
profdata <- proftools::readProfileData("Rprof.out")
proftools:plotProfileCallGraph(profdata)
#
```
...
# YOUR CODE GOES HERE
# R version
# Memory limit (Memory Available for R)
#CPU Cores
#Parallel Processing
#
```
...
# Keyboard shortcuts
# Code Snippets
# Package Management (devtools // remotes)
# Automate testing (testthat)
#
```
...
plot(res_io)
# I rank amongst the lowest in most comparations. I should probably change my computer.
```
...
loop <- function(x){
  for (i in seq_len(x)) x[i] = x[i] + 1
}

microbenchmark::microbenchmark(sum(x), loop(x))
#
```
