...
Name:  Alvaro Diez de Rivera De Solis
ICAI ID: 201801348
Collaborated with: Alvaro Lastra Aragones
...
# YOUR CODE GOES HERE
microbenchmark::microbenchmark(cs_for <- function(x) {
  for (i in x) {
    if (i == 1) {
      xc = x[i]
    } else {
...
microbenchmark::microbenchmark(cs_apply <- function(x) {
  sapply(x, function(x) sum(1:x))
})

microbenchmark::microbenchmark(cumsum(x))

#The fastest is 0.4 ns seconds faster than the second one (using the mean).
```
...
microbenchmark::microbenchmark(cumsum(x), time = 1)

#The difference has increase to 6 ns.
```
...
#The bottleneck seems to be in the plotting, and it makes sense that it is the hardest part to compute.
```
...
# YOUR CODE GOES HERE
summaryRprof()
#It takes 1.44 for the sq.var function while only 0.8 to the my.sum function.
...
microbenchmark::microbenchmark(x = x + 1)
